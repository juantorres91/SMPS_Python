...
def block(b_list, Root): 
    ...
    """ 
    This method analyses the BLOCK section from .sto file
    
    Parameters
    ----------
    b_list : list   
      List of elements from BLOCK section
    Root : Object
      Deterministic model 
    ...
    for u in b_list:       
        ...
        if u[0] == 'BL':
            ...
            if name == cr:
                ...
                if per != ct:
                    raise NameError("Realizations cannot be" 
                                    + "located at multiple stages") 
                else:
                    r_count += 1
            ...
            # Case (1.2): New realization
                ...
                if name in rea:
                    raise NameError('.Sto realizations are not '
                                    + 'organized properly')

                # Verification (4): Time in .tim file
                elif per not in Stages:
                    raise NameError('Stage ' + per + ' is not '
                                    + 'declared at the .tim file')

                # Verification (5): Time order
                elif Stages.index(per) < st:
                    raise NameError('.sto BLOCKS section does not'
                                    + ' follow the .tim stage order')
        ...
        # Case (2): New scenario
            ...
            if u[1] == delt['BOUNDS']:
                ...
                if len(u) != 4:
                    raise NameError(".sto BLOCKS section must "
                                    + "have 4 entries per line\n"
                                    + "(1) Bound type\n" 
                                    + "(2) Bound vector name\n"
                                    + "(3) Variable name \n" 
                                    + "(4) Realization value \n")
